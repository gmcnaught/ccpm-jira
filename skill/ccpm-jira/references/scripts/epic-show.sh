...
jira=$(grep "^jira:" "$epic_file" | head -1 | sed 's/^jira: *//')
...
[ -z "$jira" ] && [ $task_count -gt 0 ] && echo "  • Sync to Jira: /pm:epic-sync $epic_name"
[ -n "$jira" ] && [ "$status" != "completed" ] && echo "  • Start work: /pm:epic-start $epic_name"
